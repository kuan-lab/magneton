...
| Stage keyword(s) | Template dir | Reference templates | Pointer in root config.yaml |
|---|---|---|---|
| `pytc training` / `train affinity` / `training` | `pytorch_connectomics/configs/` | `Isotropic-Neuron-Base.yaml` (MODEL/DATASET/SOLVER/AUGMENTOR), `Isotropic-Neuron-Affinity-UNet.yaml` (override) | `affinity_prediction.config_base` / `affinity_prediction.config_file` |
| `pytc inference` / `run inference` / `affinity inference` | `pytorch_connectomics/configs/` | `hpc.yaml`, `hpc_b.yaml`, `hpc_c.yaml`, `hpc_f.yaml`, `hpc_f_bouton.yaml` | `affinity_prediction.hpc` (and `affinity_prediction.checkpoint` for the model weights) |
