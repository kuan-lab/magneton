...
from magneton.toolkit.utils.config import load_global_config_path
...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    lines += [
        f"{python_bin} -m magneton.toolkit.tools.crop "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    """Generate and submit SLURM job for crop."""
    hpc = stage_cfg.get("hpc", {})
    if not hpc.get("enable", False):
        print("[INFO] crop.hpc.enable=false, HPC submission is disabled.")
        return

    scheduler = hpc.get("scheduler", "slurm").lower()
    job_dir = hpc.get("job_dir", "./jobs/crop")
...
def crop_volume_hpc(global_cfg, restart=False, dry_run=False):
