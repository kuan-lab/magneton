...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    lines += [
        f"{python_bin} -m magneton.toolkit.tools.merge "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    Process a set of block indices locally on nodes using toolkit.tools.convert_prec.
    """
    ...
    job_dir = hpc.get("job_dir", "./jobs/merge")
...
def merge_volume_hpc(global_cfg, restart=False, dry_run=False):
