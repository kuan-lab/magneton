...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    # cfg_path = global_cfgs.get("toolkit/resize_tif", "magneton/toolkit/configs/config_resize_tif.yaml")
    cfg_path = (
        global_cfgs.get("toolkit", {})
                .get("resize_tif", "magneton/toolkit/configs/config_resize_tif.yaml")
    )
    lines += [
        f"{python_bin} -m magneton.toolkit.tools.resize_tif "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    Process a set of block indices locally on nodes using toolkit.tools.resize_tif.
    """
