...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    # cfg_path = global_cfgs.get("toolkit/mask_tif", "magneton/toolkit/configs/config_mask_tif.yaml")
    cfg_path = (
        global_cfgs.get("toolkit", {})
                .get("mask_tif", "magneton/toolkit/configs/config_mask_tif.yaml")
    )
    lines += [
        f"{python_bin} -m magneton.toolkit.tools.mask_tif "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    Process a set of block indices locally on nodes using toolkit.tools.mask_tif.
    """
