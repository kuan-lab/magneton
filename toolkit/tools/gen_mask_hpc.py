...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    # cfg_path = global_cfgs.get("toolkit/gen_mask", "magneton/toolkit/configs/config_gen_mask.yaml")
    cfg_path = (
        global_cfgs.get("toolkit", {})
                .get("gen_mask", "magneton/toolkit/configs/config_gen_mask.yaml")
    )
    lines += [
        f"{python_bin} -m magneton.toolkit.tools.gen_mask "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    Process a set of block indices locally on nodes using toolkit.tools.gen_mask.
    """
