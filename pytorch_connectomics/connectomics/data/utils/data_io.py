...
def readprecomputed(filename, mip=0, roi=[], fill_missing=True, bounded=True, progress=True):
    vol = CloudVolume(filename, mip=mip, fill_missing=fill_missing, bounded=bounded, progress=progress)
    if roi: # (Cs,Ce,Zs,Ze,Ys,Ye,Xs,Xe)
        return vol[roi[0]:roi[1], roi[2]:roi[3], roi[4]:roi[5], roi[6]:roi[7]]
    else:
        return vol[:]

def readvol(filename: str, dataset: Optional[str]=None, drop_channel: bool=False, mip: int=0, roi: list=[]):
    r"""Load volumetric data in HDF5, TIFF or PNG formats.
    """
    img_suf = filename[filename.rfind('.')+1:]
    if img_suf in ['h5', 'hdf5']:
        data = readh5(filename, dataset)
    elif 'precomputed' in img_suf:
        data = readprecomputed(filename.split(".precomputed")[0], mip, roi)
    elif 'tif' in img_suf:
